...
GResource *_g_test1_resource = NULL;

static void _g_test1_unregister_resource (void)
{
  if (_g_test1_resource)
    {
      g_resources_unregister (_g_test1_resource);
      g_resource_unref (_g_test1_resource);
      _g_test1_resource = NULL;
    }
}

static void _g_test1_register_resource (void)
{
  if (_g_test1_resource == NULL)
    {
      GBytes *bytes = g_bytes_new_static (_g_test1_resource_data.data, sizeof (_g_test1_resource_data.data));
      _g_test1_resource = g_resource_new_from_data (bytes, NULL);
      if (_g_test1_resource)
        g_resources_register (_g_test1_resource);
       g_bytes_unref (bytes);
    }
}
...
{
  _g_test1_register_resource ();
}

static void resource_destructor (void)
{
  _g_test1_unregister_resource ();
}
