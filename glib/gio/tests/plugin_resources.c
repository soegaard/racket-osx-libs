...
GResource *_g_plugin_resource = NULL;

static void _g_plugin_unregister_resource (void)
{
  if (_g_plugin_resource)
    {
      g_resources_unregister (_g_plugin_resource);
      g_resource_unref (_g_plugin_resource);
      _g_plugin_resource = NULL;
    }
}

static void _g_plugin_register_resource (void)
{
  if (_g_plugin_resource == NULL)
    {
      GBytes *bytes = g_bytes_new_static (_g_plugin_resource_data.data, sizeof (_g_plugin_resource_data.data));
      _g_plugin_resource = g_resource_new_from_data (bytes, NULL);
      if (_g_plugin_resource)
        g_resources_register (_g_plugin_resource);
       g_bytes_unref (bytes);
    }
}
...
{
  _g_plugin_register_resource ();
}

static void resource_destructor (void)
{
  _g_plugin_unregister_resource ();
}
