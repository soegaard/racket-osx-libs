#include <gio/gio.h>

#if defined (__ELF__) && ( __GNUC__ > 2 || (__GNUC__ == 2 && __GNUC_MINOR__ >= 6))
# define SECTION __attribute__ ((section (".gresource.g_test2"), aligned (8)))
#else
# define SECTION
#endif

static const SECTION union { const guint8 data[180]; const double alignment; void * const ptr;}  _g_test2_resource_data = { {
  
} };

GResource *_g_test2_resource = NULL;

void _g_test2_unregister_resource (void)
{
  if (_g_test2_resource)
    {
      g_resources_unregister (_g_test2_resource);
      g_resource_unref (_g_test2_resource);
      _g_test2_resource = NULL;
    }
}

void _g_test2_register_resource (void)
{
  if (_g_test2_resource == NULL)
    {
      GBytes *bytes = g_bytes_new_static (_g_test2_resource_data.data, sizeof (_g_test2_resource_data.data));
      _g_test2_resource = g_resource_new_from_data (bytes, NULL);
      if (_g_test2_resource)
        g_resources_register (_g_test2_resource);
       g_bytes_unref (bytes);
    }
}
